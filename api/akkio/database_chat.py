...
class MultiDatabaseAgent:
                ...
                try:
                    ...
                    if not tables_data:
                        try:
                            ...
                            tables_data = [str(n) for n in names]
    ...
    async def generate_sql_query(self, user_message: str, database, db_type: str) -> Optional[str]:
...
# -----------------------------------------------------------------------------
database_chat_router = APIRouter()

# Initialize the multi-database agent for this router
multi_db_agent = MultiDatabaseAgent()


@database_chat_router.post("/api/database_chat", response_model=ChatResponse)
...
# -------------------- Database Table -> CSV Endpoint --------------------
# Saves data into ./uploads so Explore can discover it automatically
def _get_engine(db_type: str, host: str, database: str, username: str, password: str):
...
def _get_table_data(table_name: str, db_type: str, host: str, database: str, username: str, password: str) -> pd.DataFrame:
    engine = _get_engine(db_type, host, database, username, password)
    df = pd.read_sql_table(table_name, engine)
    return df


@database_chat_router.post("/api/get_table_from_database")
async def read_data_from_db(
    ...
    try:
        df = _get_table_data(tablename, db_type, host, database, username, password)
        if df.empty:
            return JSONResponse(content={"detail": "Table is empty or not found"}, status_code=404)

        # Validate JSON serializability
        _ = json.loads(df.to_json(orient='records', date_format='iso', default_handler=str))

        # Save shared CSV (fallback path used by load_dataset)
        df.to_csv('data.csv', index=False)

        # Save table-specific CSV in ./uploads (picked as latest by load_dataset)
        os.makedirs("uploads", exist_ok=True)
        unique_name = f"{tablename.lower()}_{uuid.uuid4().hex[:8]}.csv"
        csv_path = os.path.join("uploads", unique_name)
