...
3. Use same-origin backend URL: ((typeof window !== 'undefined' && window.location.pathname.startsWith('/app/')) ? (window.location.origin + '/api/apps/' + window.location.pathname.split('/')[2]) : (process.env.REACT_APP_BACKEND_URL || process.env.VITE_BACKEND_URL || window.location?.origin || '')).trim()
